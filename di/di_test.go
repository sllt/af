...
func TestNameOf(t *testing.T) {
	...
	is.Equal("github.com/sllt/af/di.eagerTest", NameOf[eagerTest]())
	is.Equal("*github.com/sllt/af/di.eagerTest", NameOf[*eagerTest]())
	is.Equal("*map[int]bool", NameOf[*map[int]bool]())
	is.Equal("github.com/sllt/af/di.Service[int]", NameOf[Service[int]]())
}
...
func TestProvide(t *testing.T) {
	...
	s1, ok1 := i.self.services["*github.com/sllt/af/di.test"]
	is.True(ok1)
	if ok1 {
		s, ok := s1.(Service[*test])
		is.True(ok)
		if ok {
			is.Equal("*github.com/sllt/af/di.test", s.getName())
		}
	}

	s2, ok2 := i.self.services["github.com/sllt/af/di.test"]
	is.True(ok2)
	if ok2 {
	...
	_, ok3 := i.self.services["github.com/sllt/af/di.*plop"]
	is.False(ok3)
...
func TestOverride(t *testing.T) {
	...
	is.NotPanics(func() {
		...
		// OverrideNamed(i, "*github.com/sllt/af/di.test", func(i Injector) (*test, error) {
		// 	return &test{2}, nil
		// })
		...
		// OverrideNamedValue(i, "*github.com/sllt/af/di.test", &test{4})
...
func TestInvoke(t *testing.T) {
	...
	s0a, ok0a := i.self.services["github.com/sllt/af/di.test"]
	is.True(ok0a)
...
func TestInvokeStruct(t *testing.T) {
	...
	type namedDependencyButTypeMismatch struct {
		EagerTest *int `do:"*github.com/sllt/af/di.eagerTest"`
	}
	test7, err := InvokeStruct[namedDependencyButTypeMismatch](i)
	is.Equal("DI: field `github.com/sllt/af/di.namedDependencyButTypeMismatch.EagerTest` is not assignable to service *github.com/sllt/af/di.eagerTest", err.Error())
	is.Nil(test7)
