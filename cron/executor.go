...
type executor struct {
	...
	limitModeFuncRunning    *atomic.Int64    // tracks the count of limited mode funcs running
...
func newExecutor() executor {
	e := executor{
		...
		limitModeFuncRunning: atomic.NewInt64(0),
...
func (e *executor) run() {
	for {
		...
		case f := <-e.jobFunctions:
			...
			if e.limitModeMaxRunningJobs > 0 {
				countRunning := e.limitModeFuncRunning.Load()
