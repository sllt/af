...
type Option struct {
	...
	// Examples can be found in `cp_field_name_mapping_test.go`.
...
func CopyWithOption(toValue interface{}, fromValue interface{}, opt Option) (err error) {
	return cp(toValue, fromValue, opt)
}

func cp(toValue interface{}, fromValue interface{}, opt Option) (err error) {
	var (
		isSlice    bool
		amount     = 1
	...
	if from.Kind() != reflect.Slice && fromType.Kind() == reflect.Map && toType.Kind() == reflect.Map {
		...
		for _, k := range from.MapKeys() {
			...
			if !isSet {
				if err = cp(toValue.Addr().Interface(), from.MapIndex(k).Interface(), opt); err != nil {
					return err
				}
			}
		...
		if fromType.ConvertibleTo(toType) {
			for i := 0; i < from.Len(); i++ {
				...
				if !isSet {
					...
					err = cp(to.Index(i).Addr().Interface(), from.Index(i).Interface(), opt)
					if err != nil {
						continue
					}
				}
				...
				if fromField := fieldByNameOrZeroValue(source, srcFieldName); fromField.IsValid() && !shouldIgnore(fromField, fieldFlags, opt.IgnoreEmpty) {
					...
					if toField.IsValid() {
						if toField.CanSet() {
							...
							if !isSet {
								if err := cp(toField.Addr().Interface(), fromField.Interface(), opt); err != nil {
									return err
								}
							}
							if fieldFlags != 0 {
		...
		if isSlice && to.Kind() == reflect.Slice {
			if dest.Addr().Type().AssignableTo(to.Type().Elem()) {
				if to.Len() < i+1 {
					...
					if !isSet {
						// ignore error while copy slice element
						err = cp(to.Index(i).Addr().Interface(), dest.Addr().Interface(), opt)
						if err != nil {
							continue
						}
					}
				...
				if to.Len() < i+1 {
					...
					if !isSet {
						// ignore error while copy slice element
						err = cp(to.Index(i).Addr().Interface(), dest.Interface(), opt)
						if err != nil {
							continue
						}
					}
